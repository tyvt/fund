...
| 模块 | 代码 | 名称 | 策略类型 |
|------|------|------|----------|
| 红利 | H30269 | 中证红利低波动 | 仅买入，长期持有 |
| 宽基 | 000852 | 中证1000 | 买入 + 估值/移动止盈 |
| 宽基 | 000688 | 科创50 | 买入 + 移动止盈 |
| 创业板 | 399006 | 创业板指 | 买入 + 移动止盈/估值 |
| 美股 | NDX | 纳斯达克100 | 仅买入，长期持有 |
| 美股 | SPX | 标普500 | 仅买入，长期持有 |
...
两套回测**均按买入信号日触发**（非每日定投）。**默认不设买入冷却期**（`BUY_COOLDOWN_ENABLED=false`）：买入频次由各指数硬门槛自然决定，再通过单次金额与总投入控制仓位。分工如下：

| 脚本 | 输出 | 区间默认 | 含义 |
|------|------|----------|------|
| `backtest_buy_signals.py` | `inception_present.md/html` | 各指数**基日**→最新 | 仅验证买入择时：信号日累加买入并持有，**不含止盈卖出** |
| `backtest_trade_signals.py` | `trade_inception_present.md/html` | **2015-01-01**→最新 | 完整策略：含分批止盈/估值卖点；红利与美股为「仅买」对照组 |
...
对「仅买」指数（红利/美股），在**相同区间、相同金额模式**下，买入次数应一致。

```bash
# 买入信号全量回测（默认：各指数基准 100 元 + 当日涨跌缩放）
...
|------|------|---------------:|-------:|-----:|----------|
| 红利 | H30269 | 2,816 | 405 | 14.4% | 约每 7 个交易日 1 次 |
| 中证1000 | 000852 | 2,816 | 284 | 10.1% | 约每 10 个交易日 1 次 |
| 科创50 | 000688 | 1,463 | 203 | 13.9% | 约每 7 个交易日 1 次 |
| 创业板 | 399006 | 3,909 | 232 | 5.9% | 约每 17 个交易日 1 次 |
| 纳指100 | NDX | 4,171 | 618 | 14.8% | 约每 7 个交易日 1 次 |
| 标普500 | SPX | 3,417 | 417 | 12.2% | 约每 8 个交易日 1 次 |

取消冷却后，硬门槛连续达标日会连买。控制总投入靠**基准金额（默认 100）+ 当日涨跌缩放**；纳指/标普实盘常受 QDII 每日限购约束。
...
```
实际买入 = 基准金额 × clamp(1 − 敏感度 × 当日涨跌幅, 下限, 上限)
```

- 基准默认各指数 **100 元**（可用 `{代码}_BUY_AMOUNT` 或 `BUY_AMOUNT_DEFAULT` 覆盖）
- 默认敏感度 **10**：跌 3% → 1.30×（130 元）；涨 3% → 0.70×（70 元）
- 系数限制在 **0.5×–2.0×**（即约 50–200 元）
- 无昨收/涨跌数据时按 1.0×（等于基准）
...
| 代码 | 指数 | 默认基准 |
|------|------|--------:|
| NDX / SPX / 399006 / H30269 / 000688 / 000852 | 全部 | **100** |

美股 QDII 若日限约 100 元，单日两市合计可能触及限购；回测按信号全额计入，实盘请自行控制。

#### 环境变量（`push.env`）

| 变量 | 默认 | 说明 |
|------|------|------|
| `BUY_AMOUNT_DEFAULT` | `100` | 各指数默认基准 |
| `{代码}_BUY_AMOUNT` | `100` | 覆盖单只指数基准 |
...
| `REMAINING_INVESTMENT_BUDGET` | `50000` | 当年剩余可用额度（展示用） |

### 买卖波段回测

按当前买入/卖出标准模拟波段交易（触发卖点时清仓）。**买入为信号触发、默认无冷却期**，与买入信号回测一致。**长期持有、无卖出**：红利（H30269）、美股（NDX/SPX）。**其余均探索止盈**：A 股宽基两只、创业板。

默认区间自 **2015-01-01**（可用 `--start` 覆盖）。

```bash
python backtest_trade_signals.py
```

结果写入 `output/backtest/trade_inception_present.md`（含买卖次数、收益对比及卖点日期）。报告中的「买卖日期」节可核对卖点；买入信号日可用 `backtest_buy_signals.py --list-dates` 对照。
...
| `buy_amount_*.py` | 买入金额：涨跌缩放、预算、排名（可选）、报告展示 |
...
| `cache/` | 外部数据本地缓存：A 股（`cache/cn/`）、创业板（`cache/cyb/`）、美股（`cache/us/`），按日刷新。 |
...
| 指数 | 代码 | 买入条件（须全部满足） |
|------|------|------------------------|
| 中证红利低波动 | H30269 | 利差 **> 2.8%**；利差分位 **≥ 40%**；PE 分位 **≤ 74%**；距 90 日低点 **≤ 6%**；近1年区间 **≤ 62%**；高点回撤 **≥ 10%**；股息可持续性（PE **≥ 5**；252 日股息率飙升 **≤ 1.5×**，近1年区间 **≤ 10%** 时豁免飙升检查） |
...
2. **估值卖点**（中证1000 为主）：PE 分位 **≥ 82%** 且距低点 **≥ 24%**；或浮盈 **≥ 40%** 后自峰值回撤 **≥ 10%**（移动止盈兜底）。可选近1年区间高位 + PE 组合门槛。
3. 卖出与买入互斥：当日若仍满足买入条件，不触发卖出。

| 指数 | 代码 | 买入（分位窗口约 10 年） | 卖出 |
|------|------|--------------------------|------|
| 中证1000 | 000852 | 利差 **≥ 70%**；PE **≤ 52%**；PB **≤ 56%**；距低点 **≤ 5%**；近1年区间 **≤ 34%**；高点回撤 **≥ 16%** | 估值：PE **≥ 82%** 且距低点 **≥ 24%**；兜底：浮盈 **≥ 40%** 后回撤 **≥ 10%** |
...
| **卖出** | 分批：浮盈 **50%** 卖 **1/3**、**100%** 再卖 **1/3**；余 **1/3** 移动止盈回撤 **≥ 12%**；或 PE/PB 分位均 **≥ 78%**；或 PEG(5年) **≥ 3.0** 且估值不低 |
...
- **卖出**：无（长期持有型）；建议通过场外 QDII 基金执行，注意限购。
- **环境变量前缀**：`NDX_*` / `SPX_*`。
...
| 宽基 | 2 | 股债利差分位 + PE/PB 分位 + 价格位置（评分制） | 移动止盈；中证1000 另加估值卖点 |
...
高波动品种（科创50、创业板）采用**三档**而非一次性清仓；中证1000 以估值卖点为主、移动止盈兜底：

| 品种 | 浮盈 50% | 浮盈 80% / 100% | 剩余仓位 |
|------|----------|-----------------|----------|
| 科创50 | 卖出初始仓位 1/3 | 再卖 1/3（80%） | 最后 1/3 移动止盈（回撤 12.5%） |
| 创业板 | 卖出 1/3 | 再卖 1/3（100%） | 最后 1/3 移动止盈（回撤 12%） |
| 中证1000 | — | — | 估值卖点为主；浮盈 ≥40% 后回撤 ≥10% 兜底 |

回测见 `backtest_trade_signals.py` → `trade_inception_present.md`；实现见 `sell_trailing.simulate_trades_trailing`。估值类卖点（中证1000、创业板 PE/PB）与分批止盈可叠加。

**分批卖出后的行为**（当前实现）：

- 同一持仓内，各浮盈档位（50%/80%/100%）**仅触发一次**（`stages_triggered` 记录）；不会在震荡中于同一档位反复卖出。
...
**组合再平衡**：系统不自动再平衡。组合权重定义的是单次买入金额比例；因各指数触发频率不同，持仓市值会漂移——请按年度或主观风险偏好**手动再平衡**。
