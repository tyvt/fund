...
MIN_ROE_PCT = _env_float("DLV_MIN_ROE_PCT", 11.0)
MIN_PROFIT_YOY_PCT = _env_float("DLV_MIN_PROFIT_YOY_PCT", -10.0)
OCF_QUALITY_FILTER_ENABLED = _env_bool("DLV_OCF_QUALITY_FILTER_ENABLED", True)
MIN_OCF_TO_PROFIT = _env_float("DLV_MIN_OCF_TO_PROFIT", 0.8)
...
# 排雷/增强因子：默认软性评分扣减，替代硬性剔除（绝对质量底线仍硬过滤）
SOFT_RISK_SCORING_ENABLED = _env_bool("DLV_SOFT_RISK_SCORING_ENABLED", True)
SOFT_ENHANCED_SCORING_ENABLED = _env_bool("DLV_SOFT_ENHANCED_SCORING_ENABLED", True)
RISK_PENALTY_OCF = _env_float("DLV_RISK_PENALTY_OCF", 3.0)
...
ENH_PENALTY_TRAP = _env_float("DLV_ENH_PENALTY_TRAP", 4.0)
ENH_PENALTY_MOMENTUM = _env_float("DLV_ENH_PENALTY_MOMENTUM", 2.0)
ENH_PENALTY_STABILITY = _env_float("DLV_ENH_PENALTY_STABILITY", 2.0)
ENH_PENALTY_COVERAGE = _env_float("DLV_ENH_PENALTY_COVERAGE", 2.5)
...
# --- 估值买入偏好（PB 低于行业中位数优先）---
VALUATION_BUY_ENABLED = _env_bool("DLV_VALUATION_BUY_ENABLED", True)
VALUATION_PB_QUANTILE_NORMAL_PCT = _env_float("DLV_VALUATION_PB_QUANTILE_NORMAL_PCT", 50.0)
VALUATION_PB_QUANTILE_TIGHT_PCT = _env_float("DLV_VALUATION_PB_QUANTILE_TIGHT_PCT", 30.0)
...
# --- 增强因子：可持续股息 / 利差陷阱 / 盈利动量 / Beta / 质量 ---
SUSTAINABLE_DIVIDEND_ENABLED = _env_bool("DLV_SUSTAINABLE_DIVIDEND_ENABLED", True)
YIELD_SPREAD_PERCENTILE_ENABLED = _env_bool("DLV_YIELD_SPREAD_PERCENTILE_ENABLED", True)
YIELD_SPREAD_PERCENTILE_TRAP = _env_float("DLV_YIELD_SPREAD_PERCENTILE_TRAP", 92.7)
YIELD_SPREAD_LOOKBACK_DAYS = _env_int("DLV_YIELD_SPREAD_LOOKBACK_DAYS", 252)
PROFIT_MOMENTUM_FILTER_ENABLED = _env_bool("DLV_PROFIT_MOMENTUM_FILTER_ENABLED", True)
PROFIT_MOMENTUM_MIN_QOQ_POSITIVE = _env_int("DLV_PROFIT_MOMENTUM_MIN_QOQ_POSITIVE", 1)
PROFIT_STABILITY_FILTER_ENABLED = _env_bool("DLV_PROFIT_STABILITY_FILTER_ENABLED", True)
MAX_PROFIT_CV = _env_float("DLV_MAX_PROFIT_CV", 0.65)
DIVIDEND_COVERAGE_FILTER_ENABLED = _env_bool("DLV_DIVIDEND_COVERAGE_FILTER_ENABLED", True)
MIN_DIVIDEND_COVERAGE = _env_float("DLV_MIN_DIVIDEND_COVERAGE", 1.27)
...
BETA_MIN_LOW_FRAC = _env_float("DLV_BETA_MIN_LOW_FRAC", 0.35)
BETA_MAX_HIGH_FRAC = _env_float("DLV_BETA_MAX_HIGH_FRAC", 0.81)


def panel_factor_cache_key() -> str:
    """Panel 构建相关参数的缓存键（优化 trial 间按参数复用）。"""
    return "|".join(
        str(v)
        for v in (
...
EXPECTED_DIVIDEND_YIELD_ENABLED = _env_bool("DLV_EXPECTED_DIVIDEND_YIELD_ENABLED", True)
EXPECTED_DIVIDEND_LOOKBACK_YEARS = _env_int("DLV_EXPECTED_DIVIDEND_LOOKBACK_YEARS", 3)
...
INDEX_RETENTION_LIQUIDITY_ENABLED = _env_bool("DLV_INDEX_RETENTION_LIQUIDITY_ENABLED", True)
INDEX_RETENTION_LIQUIDITY_PERCENTILE = _env_float("DLV_INDEX_RETENTION_LIQUIDITY_PERCENTILE", 0.90)
INDEX_RETENTION_LIQUIDITY_LOOKBACK_DAYS = _env_int("DLV_INDEX_RETENTION_LIQUIDITY_LOOKBACK_DAYS", 252)
